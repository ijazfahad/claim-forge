...
import { TEST_CASES, TEST_CASES_BY_CATEGORY, getTestCaseById, getTestCasesByTag } from '../test/test-cases';
import { claimStorage } from '../services/claim-storage';
...
/**
 * GET /api/test/cases
 * Get all test cases or filter by category/tag
 */
router.get('/cases', async (req, res) => {
  try {
    ...
    if (category && typeof category === 'string') {
      testCases = TEST_CASES_BY_CATEGORY[category as keyof typeof TEST_CASES_BY_CATEGORY] || [];
    }
    ...
    res.json({
      ...
      data: {
        ...
        pagination: {
          ...
          has_more: (parseInt(offset as string) + parseInt(limit as string)) < testCases.length
        }
      }
    });
  } catch (error) {
...
router.post('/run/:id', async (req, res) => {
  try {
    ...
    const recordId = await claimStorage.storeClaimValidation(validationRecord);
...
router.post('/run-batch', async (req, res) => {
  try {
    ...
    if (test_case_ids && Array.isArray(test_case_ids)) {
      testCases = test_case_ids.map(id => getTestCaseById(id)).filter((tc): tc is NonNullable<typeof tc> => tc !== undefined);
    } else if (category) {
      testCases = TEST_CASES_BY_CATEGORY[category as keyof typeof TEST_CASES_BY_CATEGORY] || [];
    } else if (tag) {
...
router.get('/stats', async (req, res) => {
  try {
    ...
    const stats = await claimStorage.getValidationStats();
...
/**
 * GET /api/test/validations
 * Get stored validation records
 */
router.get('/validations', async (req, res) => {
  try {
    ...
    const validations = await claimStorage.getClaimValidations(
...
/**
 * GET /api/test/validations/:claimId
...
router.get('/validations/:claimId', async (req, res) => {
  try {
    ...
    const validation = await claimStorage.getClaimValidation(claimId);
    ...
    const steps = await claimStorage.getValidationSteps(validation.id);
