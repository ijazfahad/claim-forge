...
import { RetryResult } from './retry-agent';
...
export class EvaluateAgent extends BaseAgent {
...
`;
  ...
  /**
   * Evaluate all research results and create final validation payload
   */
...
`;
   ...
   */
  private calculateRiskScore(questionResults: any[]): number {
   ...
   */
  private determineStatus(questionResults: any[], riskScore: number): 'GO' | 'NO_GO' | 'REVIEW_REQUIRED' {
