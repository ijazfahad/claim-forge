...
export class ClaimStorageService {
  ...
  async storeClaimValidation(record: Omit<ClaimValidationRecord, 'id' | 'created_at' | 'updated_at'>): Promise<string> {
    if (!this.client) throw new Error('Database client not initialized');
  ...
  async updateValidationStep(
    ...
    stepName: string,
    stepData: Partial<ValidationStep>
  ): Promise<void> {
    if (!this.client) throw new Error('Database client not initialized');
  ...
  async getClaimValidations(
    limit: number = 100,
    offset: number = 0,
    status?: string
  ): Promise<ClaimValidationRecord[]> {
    if (!this.client) throw new Error('Database client not initialized');
  ...
  async getValidationSteps(claimValidationId: string): Promise<ValidationStep[]> {
    if (!this.client) throw new Error('Database client not initialized');
...
// Singleton instance
export const claimStorage = new ClaimStorageService();
